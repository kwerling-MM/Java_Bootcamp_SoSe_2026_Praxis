...
public class JorthTest {
    ...
    @Test
    void dupDuplicatesInteger() {
        assertEquals("[3, 3]", jorth.run("3 dup"));
    }

    @Test
    void dupDuplicatesString() {
        assertEquals("[ab, ab]", jorth.run("\"ab\" dup"));
    }

    @Test
    void dupWithAdditionWorks() {
        // 3 dup + -> 3 3 + -> 6
        assertEquals("[6]", jorth.run("3 dup +"));
    }

    @Test
    void dupOnEmptyStackDoesNothing() {
        assertEquals("[]", jorth.run("dup"));
    }

    @Test
    void multipleDupCommands() {
        assertEquals("[5, 5, 5, 5]", jorth.run("5 dup dup dup"));
    }

    @Test
    void dupAfterComplexOperation() {
        assertEquals("[abcd, abcd]", jorth.run("\"ab\" \"cd\" + dup"));
    }

    @Test
    void lessThanWorks() {
        assertEquals("[1]", jorth.run("3 4 <"));
        assertEquals("[0]", jorth.run("4 3 <"));
        assertEquals("[0]", jorth.run("3 3 <"));
    }

    @Test
    void greaterThanWorks() {
        assertEquals("[0]", jorth.run("3 4 >"));
        assertEquals("[1]", jorth.run("4 3 >"));
        assertEquals("[0]", jorth.run("3 3 >"));
    }

    @Test
    void equalsWorks() {
        assertEquals("[1]", jorth.run("3 3 =="));
        assertEquals("[0]", jorth.run("3 4 =="));
    }
    ...
    @Test
    void comparisonWithNonIntegersFailsGracefully() {
        // Wenn ein String dabei ist, bleiben beide auf dem Stack
        assertEquals("[abc, 3]", jorth.run("\"abc\" 3 =="));
        assertEquals("[3, abc]", jorth.run("3 \"abc\" <"));
    }

    @Test
    void comparisonWithEmptyStackDoesNothing() {
        assertEquals("[1]", jorth.run("1 <")); // Zu wenig Elemente
        assertEquals("[]", jorth.run("=="));   // Stack leer
    }

    @Test
    void ifTrueBranchExecutes() {
        assertEquals("[yes]", jorth.run("1 if \"yes\" else \"no\" endif"));
    }

    @Test
    void ifFalseBranchExecutesElse() {
        assertEquals("[no]", jorth.run("0 if \"yes\" else \"no\" endif"));
    }

    @Test
    void ifWithoutElseTrue() {
        assertEquals("[42]", jorth.run("1 if 42 endif"));
    }

    @Test
    void ifWithoutElseFalse() {
        assertEquals("[]", jorth.run("0 if 42 endif"));
    }

    @Test
    void comparisonCombinedWithIf() {
        assertEquals("[less]", jorth.run("3 4 < if \"less\" else \"more\" endif"));
        assertEquals("[more]", jorth.run("10 4 < if \"less\" else \"more\" endif"));
    }

    @Test
    void complexLogicInsideIf() {
        // Wenn 1, dann rechne 3 + 4, sonst 10 - 2
        assertEquals("[7]", jorth.run("1 if 3 4 + else 10 2 - endif"));
        assertEquals("[8]", jorth.run("0 if 3 4 + else 10 2 - endif"));
    }

    @Test
    void ifOnEmptyStackDefaultsToFalse() {
        assertEquals("[skipped]", jorth.run("if \"inside\" else \"skipped\" endif"));
    }
}